...
# 第1章 Foundations {#chapter-01-foundations}

## 1.1 Basics {#section-01-foundations-01-basics}

#### 1.1.1 Copilot 基本操作を習得する

このファイルでは、GitHub Copilot の IDE 内での操作方法と、基本的なコンテキスト設定を学びます。

---

#### 1.1.2 ✅ コード補完（Inline Suggestions）の基本操作
...
##### 1.1.3.3 XMLコメント(Documentation)の活用
...
## 1.2 Comment Driven Dev {#section-01-foundations-02-comment-driven-dev}

#### 1.2.1 効果的なコメントで Copilot の精度を上げる
...
#### 1.2.4 🔧 XMLコメント (Documentation Comments) の活用
...
## 1.3 Beginner Scenarios {#section-01-foundations-03-beginner-scenarios}

#### 1.3.1 初心者が最初に取り組むべき 3 つの実装シナリオ

実際のコード例を通じて、Copilot の活用法を学びます。各シナリオは簡潔な実装から段階的に複雑化します。

---

#### 1.3.2 シナリオ 1: ファイル操作の実装

##### 1.3.2.1 状況
「テキストファイルを読み込んで、各行をリストとして返す機能を作りたい」

##### 1.3.2.2 ステップ 1: 意図をコメントで伝える
...
#### 1.3.3 シナリオ 2: LINQ の使い方を学ぶ

##### 1.3.3.1 状況
「コレクション操作を効率的に行いたい。フォアーチループではなく LINQ を使いたい」

##### 1.3.3.2 ステップ 1: 基本的な foreach 版
...
##### 1.3.3.3 ステップ 2: Copilot に LINQ 版を提案してもらう
...
#### 1.3.6 💡 各シナリオから学べることの活用先

##### 1.3.6.1 シナリオ 1（ファイル操作）から学べること
...
# 第2章 Core Features {#chapter-02-core-features}

## 2.1 Inline Completions {#section-02-core-features-01-inline-completions}

#### 2.1.1 コード補完のテクニックをマスターする

Inline Completions はもっとも頻繁に使う機能です。コメント駆動開発と組み合わせて、精度を最大化します。

---

#### 2.1.2 💡 テクニック 1: コメント駆動開発

##### 2.1.2.1 パターン：具体的なコメント = 精度の高い提案
...
## 2.2 Copilot Chat {#section-02-core-features-02-copilot-chat}

#### 2.2.1 チャットインターフェースで対話的に支援を受ける

Copilot Chat は単なるコード生成ではなく、「相談」のツール。設計から実装、改善まで幅広く活用できます。

---

#### 2.2.2 💡 テクニック 1: コード説明を依頼

##### 2.2.2.1 シーン：複雑なコードの意味を理解したい
...
#### 2.2.3 💡 テクニック 2: リファクタリングの提案

##### 2.2.3.1 シーン：コードが複雑で読みづらい
...
#### 2.2.5 💡 テクニック 4: ドキュメント生成

##### 2.2.5.1 シーン：XMLコメントを自動生成したい
...
## 2.3 Inline Edits {#section-02-core-features-03-inline-edits}

#### 2.3.1 選択したコードを直接修正する

Inline Edits は「選択したコード」に対して修正指示を出す機能。範囲を限定した改善が可能です。

---

#### 2.3.2 💡 テクニック 1: コードの最適化

##### 2.3.2.1 シーン：このコード遅いから最適化したい
...
#### 2.3.3 💡 テクニック 2: エラーハンドリングの追加

##### 2.3.3.1 シーン：このコード例外処理がない
...
## 2.4 Agent Mode {#section-02-core-features-04-agent-mode}

#### 2.4.1 複数ファイルにまたがる大規模タスクを自動実装

Agent Mode は Copilot のもっとも高度な機能。複数ファイルを自動生成・修正して、大規模な機能追加やリファクタリングを一度に実行できます。

---

#### 2.4.2 💡 テクニック 1: 新機能の追加

##### 2.4.2.1 シーン：ユーザー認証機能を一から追加したい
...
#### 2.4.3 💡 テクニック 2: アーキテクチャリファクタリング

##### 2.4.3.1 シーン：データアクセス層を Repository パターンにリファクタリング
...
#### 2.4.4 💡 テクニック 3: テスト自動生成

##### 2.4.4.1 シーン：このクラスのユニットテストを生成したい
...
# 第3章 Hands on Workflows {#chapter-03-hands-on-workflows}

## 3.1 Design Patterns {#section-03-hands-on-workflows-01-design-patterns}

#### 3.1.1 既存コードにパターンを適用してリファクタリング

中級者にとって重要な「デザインパターンを実務に適用する」スキルを learning-by-doing で習得します。

---

#### 3.1.2 🎯 シナリオ：支払い処理にストラテジーパターンを適用

##### 3.1.2.1 状況
「複数の支払い方法（クレジットカード、PayPal、銀行振込）に対応したい」

##### 3.1.2.2 ❌ リファクタリング前：if-else で分岐
...
##### 3.1.2.3 ✅ リファクタリング後：Strategy パターン
...
##### 3.1.3.1 シーン：複数のデータソース（SQL, NoSQL, API）に対応
...
## 3.2 Performance Optimization {#section-03-hands-on-workflows-02-performance-optimization}

#### 3.2.1 遅いコードを見つけて、効率的に改善する

パフォーマンス問題の診断と改善は、中級者必須スキル。Copilot を使った効率的なプロセスを学びます。

---

#### 3.2.2 🎯 シーン 1: N+1 クエリ問題の解決
...
#### 3.2.4 🎯 シーン 3: LINQ クエリの最適化

##### 3.2.4.1 ❌ 非効率版：メモリ上でフィルタリング
...
#### 3.2.6 🔧 Copilot を使った最適化フロー

##### 3.2.6.1 ステップ 1: 遅いコードを選択

```csharp
var expensiveOrders = _context.Orders.ToList()...
```

##### 3.2.6.2 ステップ 2: Chat で質問

```
「このコードが遅い理由は？最適化方法を教えてください」
```

##### 3.2.6.3 ステップ 3: Copilot が説明
...
##### 3.2.6.4 ステップ 4: Inline Edits で修正
...
## 3.3 Testability {#section-03-hands-on-workflows-03-testability}

#### 3.3.1 テストしやすい設計にリファクタリング

「このコード、テストが書きやすい/難しい」の差は設計に由来します。依存性注入とモックを活用した設計を学びます。

---

#### 3.3.2 🎯 シーン：依存性が多いコードをテスト可能に
...
##### 3.3.3.3 原則 3: ビジネスロジックと I/O を分離
...
# 第4章 Role Mastery {#chapter-04-role-mastery}

## 4.1 Beginner Developer {#section-04-role-mastery-01-beginner-developer}

#### 4.1.1 基礎をしっかり習得して、コーディング効率を高める
...
##### 4.1.2.3 フェーズ 3：Copilot の活用パターン習得（1週間）
...
#### 4.1.3 🎯 初級者が陥りやすい落とし穴と解決策

##### 4.1.3.1 ❌ 落とし穴 1: コメントが曖昧 → Copilot が外れた提案
...
#### 4.1.4 💡 初級者向けの効率的な Copilot 使い方

##### 4.1.4.1 パターン 1: 仕様確認（Chat）
...
##### 4.1.4.2 パターン 2: リファクタリング質問（Chat）
...
## 4.2 Mid Level Developer {#section-04-role-mastery-02-mid-level-developer}

#### 4.2.1 実装スキルを深掘りして、チーム貢献度を高める
...
#### 4.2.3 🎯 中級者が習得すべき 3つのスキル

##### 4.2.3.1 スキル 1: 設計パターンの実装
...
#### 4.2.4 🚀 中級者向け実践タスク（月 1 本程度）

##### 4.2.4.1 タスク 例：既存コードの設計パターン適用
...
## 4.3 Senior Lead {#section-04-role-mastery-03-senior-lead}

#### 4.3.1 アーキテクチャ設計と組織的影響力で、事業価値を最大化する
...
##### 4.3.3.3 責務 3: パフォーマンス＆スケーラビリティの管理
...
#### 4.3.4 🎓 シニアリードの本領発揮：技術的負債の削減

##### 4.3.4.1 シーン：「新機能追加が遅い」問題
...
## 4.4 QA Engineer {#section-04-role-mastery-04-qa-engineer}

#### 4.4.1 Copilot で テスト自動化と品質保証を高度化する
...
#### 4.4.3 🎯 QA エンジニアが Copilot を活用する 3 つのシーン

##### 4.4.3.1 シーン 1: 単体テストコード生成の自動化
...
##### 4.4.3.2 シーン 2: 統合テスト・E2E テストの効率化
...
#### 4.4.4 🔧 テスト自動化フレームワークの設計

##### 4.4.4.1 Unit Test (xUnit)
...
##### 4.4.4.2 Integration Test (xUnit + Moq)
...
##### 4.4.4.3 E2E Test (WebApplicationFactory)
...
# 第5章 Team Operations {#chapter-05-team-operations}

## 5.1 Team Standards {#section-05-team-operations-01-team-standards}

#### 5.1.1 コード品質と開発効率のための共通ルールを作る
...
##### 5.1.4.2 Week 2: ドキュメント作成 & CI/CD パイプライン更新
```
...
## 5.2 Adoption Strategy {#section-05-team-operations-02-adoption-strategy}

#### 5.2.1 チーム全体で Copilot を段階的に導入し、生産性を最大化する
...
##### 5.2.3.3 フェーズ 3: 最適化と継続改善（6 か月以上）
...
## 5.3 Metrics Measurement {#section-05-team-operations-03-metrics-measurement}

#### 5.3.1 Copilot の効果を定量的に測定して、投資効果を証明する
...
##### 5.3.4.1 Weekly Dashboard（各チームが見る）
...
##### 5.3.4.2 Monthly Executive Summary
...
#### 5.3.6 ✅ Copilot 効果測定チェックリスト

##### 5.3.6.1 導入前（ベースライン測定）
...
## 5.4 Copilot Instructions Template {#section-05-team-operations-04-copilot-instructions-template}

#### 5.4.1 プロジェクト固有の Copilot ガイドラインをテンプレートして、全チームで統一的に使う
...
#### 5.4.4 ✅ Copilot Instructions 導入チェックリスト
...
# 第6章 Reference {#chapter-06-reference}

## 6.1 Troubleshooting {#section-06-reference-01-troubleshooting}

#### 6.1.1 よくあるエラーと解決策を索引形式で整理
...
## 6.2 Quick Reference {#section-06-reference-02-quick-reference}

#### 6.2.1 よく使うコマンド、パターン、キーボードショートカットを素早く参照

Ctrl+F で検索して使用。

---

#### 6.2.2 ⌨️ キーボードショートカット

##### 6.2.2.1 Copilot（全 IDE）
...
#### 6.2.5 📊 Entity Framework Core パターン

##### 6.2.5.1 Context Setup
...
##### 6.2.9.2 Code Comment (Complex logic)
...
## 6.3 Feature BY Role Matrix {#section-06-reference-03-feature-by-role-matrix}

#### 6.3.1 各ロールが「どの Copilot 機能」を「どんなシーンで」使うかのマトリックス
...
##### 6.3.4.1 シーン 1: "新しい API エンドポイント作成"
...
#### 6.3.9 📞 Questions by Role

##### 6.3.9.1 初級: 「Copilot が提案したコード、信頼していい？」
...
## 6.4 Project Templates {#section-06-reference-04-project-templates}

#### 6.4.1 よくあるプロジェクト種別ごとの「Copilot Best Practice」ガイド
...
##### 6.4.2.1 1️⃣ Web API (ASP.NET Core)
...
##### 6.4.2.2 2️⃣ Console Application / Worker Service
...
##### 6.4.2.5 5️⃣ Console App (Simple Automation / Scripts)
...
#### 6.4.3 📋 すべてのプロジェクトタイプに共通

##### 6.4.3.1 `.github/copilot-instructions.md` 必須
